  ...
  <div className="gallery-card m-2" onDragStart={onDragStart}>
    ...
    <div className="d-flex justify-content-between py-2 px-3">
      ...
      <p className="d-flex align-items-center justify-content-center">
        <i className="fa fa-heart mr-1" style={{color: 'red', fontSize: 18}} /> {data.userlikes.length}
      </p>
    </div>
  </div>
)
...
const Gallery = ({gallery, owner}) => {
  ...
  const UploadButton = () => 
    <Upload {...props}>
      <CButton color="primary" className="shadow btn-pill text-bold px-4" disabled={uploading} style={{height: 40}}>
        {uploading ? (
          <Loader />
        ) : "Add Image"}
      </CButton>
    ...
    <CCard className="shadow">
      <CCardBody className="py-4 px-5">
        <div className="d-flex align-items-center justify-content-between mb-4">
          <h3 className="text-secondary"><strong>Your Gallery</strong></h3>
        ...
        { 
          (!gallery || gallery.length === 0) ? 
            <>
              <p className="my-5">
                {owner ? "Add image Forum ipsume ...":"No gallery"}</p>
              {owner && <UploadButton/>}
            </>
            :
            <AliceCarousel 
            ...
            >
              {
                gallery.map((img, key) => <CardItem key={key} onDragStart={handleOnDragStart} data={img}/>)
              }
            </AliceCarousel>
        }
      </CCardBody>
